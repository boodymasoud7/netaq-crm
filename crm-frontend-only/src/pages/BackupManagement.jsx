...
export default function BackupManagement() {
  ...
  const loadStorageInfo = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/backups/storage', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        }
      })
  ...
  const downloadBackup = async (backupId, filename) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/backups/download/${backupId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        }
      })
  ...
  const restoreBackup = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/backups/restore/${backupId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        }
  ...
  const deleteBackup = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/backups/delete/${backupId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        }
  ...
  const uploadBackup = async (file) => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/backups/upload', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        },
  ...
  const clearAllBackups = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/backups/clear-all', {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken') || 'dev-token'}`
        }
