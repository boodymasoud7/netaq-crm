...
const WikiMapiaViewer = () => {
  ...
  const changeMapView = (view) => {
    ...
    const viewNames = {
      ...
      port: 'عرض الميناء',
      residential: 'العرض السكني'
    }
    toast.success(`🔄 تم التحويل إلى ${viewNames[view]}`)
  }

  const openInWikiMapia = () => {
    const loc = damietteLocations[currentView]
    const url = `https://wikimapia.org/#lang=ar&lat=${loc.lat}&lon=${loc.lon}&z=${loc.zoom}&m=w&search=${encodeURIComponent('دمياط الجديدة')}`
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center">
        <div className="h-12 w-12 animate-spin mx-auto text-green-500 mb-4 rounded-full border-4 border-green-200 border-t-green-500"></div>
        <p className="text-lg text-gray-600">جاري تحميل WikiMapia لدمياط الجديدة...</p>
      </div>
    )
  }

  // إحداثيات ومعلومات مناطق دمياط - محسّنة لـ WikiMapia
  ...
  const getWikiMapiaEmbedUrl = (location) => {
    ...
    // WikiMapia مع تحديد مباشر للموقع
    return `https://wikimapia.org/#lang=ar&lat=${loc.lat}&lon=${loc.lon}&z=${loc.zoom}&m=w`
  }
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-xs text-gray-500 ml-2">
              📍 {damietteLocations[currentView].lat.toFixed(4)}, {damietteLocations[currentView].lon.toFixed(4)}
            </div>
          </div>
        </div>
        ...
        <div className="relative h-[700px] border-2 border-green-200 rounded-lg overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 bg-black/70 text-white px-3 py-2 rounded-lg text-xs max-w-xs">
            ...
            <div className="mt-2 text-xs text-yellow-300 bg-black/20 px-2 py-1 rounded">
              💡 انتظر قليلاً لتحميل الخريطة وستظهر دمياط مباشرة
            </div>
          </div>
        </div>
      </Card>
