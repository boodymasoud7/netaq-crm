...
export function useSSENotificationSender() {
  ...
  const sendLeadConvertedNotification = useCallback(async (leadName) => {
    ...
    try {
      ...
      const response = await fetch('https://netaqcrm.site/api/notifications-stream/emit', {
        method: 'POST',
        headers: {
  ...
  const sendFollowUpCompletedNotification = useCallback(async (followUpTitle, clientName, result = '') => {
    ...
    try {
      ...
      const response = await fetch('https://netaqcrm.site/api/notifications/send', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
      ...
      if (response.ok) {
        ...
        try {
          await fetch('https://netaqcrm.site/api/notifications-stream/emit', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('authToken')}`
