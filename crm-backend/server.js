...
const limiter = rateLimit({
  ...
  max: process.env.NODE_ENV === 'production' ? 300 : 10000, // 10k requests in dev, 300 in prod
  skip: (req) => {
...
app.use(limiter);
app.use(cors({
  origin: ['http://localhost:3000', 'http://localhost:3001', 'http://localhost:5173', 'http://localhost:4173', process.env.FRONTEND_URL].filter(Boolean),
  credentials: true,
